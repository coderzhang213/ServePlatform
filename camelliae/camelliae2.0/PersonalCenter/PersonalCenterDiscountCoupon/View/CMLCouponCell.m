...
@interface CMLCouponCell ()<NetWorkProtocol>
...
- (void)refreshCurrentCell:(CMLMyCouponsModel *)model withIsUse:(int)isUse withProcess:(int)process {
    ...
    self.amoutLabel.frame = CGRectMake(230 * Proportion/2 - CGRectGetWidth(self.amoutLabel.frame)/2 + 10 * Proportion,
                                       CGRectGetMinY(self.dateLabel.frame) - CGRectGetHeight(self.amoutLabel.frame)/2 - 5 * Proportion,//196 * Proportion/2 - CGRectGetHeight(self.amoutLabel.frame)/2,
...
- (void)useContentButtonClicked:(UIButton *)button {
    ...
    [self.delegate useContentButtonClickedOfCouponCellWith:self.currentHeight];
...
- (UIButton *)unfoldButton {
    ...
    if (!_unfoldButton) {
        _unfoldButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bgImageView.frame) - CGRectGetHeight(self.bgImageView.frame),
                                                                  0,
