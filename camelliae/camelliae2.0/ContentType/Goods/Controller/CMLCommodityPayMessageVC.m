...
@interface CMLCommodityPayMessageVC () <NavigationBarProtocol,NetWorkProtocol,CMLUserAddressLisDelgate,AlterAddressMessageDeleagte,UITextViewDelegate,CMLNewInvoiceVCDelegate, CMLChooseCouponsViewControllerDelegate>
...
@implementation CMLCommodityPayMessageVC
...
- (void) loadMessageOfVC{
    ...
    if ([self.obj.retData.is_deposit intValue] == 1) {
        ...
        self.priceLab.text = [NSString stringWithFormat:@"¥ %0.2f", [self.obj.retData.deposit_money floatValue]*self.buyNum];
        
        self.payPrice = [NSNumber numberWithFloat:[self.obj.retData.deposit_money floatValue]*self.buyNum];
        ...
        if (self.packageID) {
            ...
            for (int i = 0; i < self.obj.retData.packageInfo.dataList.count; i++) {
                ...
                if ([costObj.currentID intValue] == [self.packageID intValue]) {
                    ...
                    self.priceLab.text = [NSString stringWithFormat:@"¥ %0.2f", [costObj.totalAmount floatValue]*self.buyNum];
                    
                    self.payPrice = [NSNumber numberWithFloat:[costObj.totalAmount floatValue]*self.buyNum];
                    
                    /*可享折扣*/
                    if ([self.obj.retData.isEnjoyDiscount intValue] == 1) {
                        if ([self.obj.retData.is_discount intValue] == 1) {/*存在折扣*/
                            if ([costObj.discount intValue] != 0) {
                                self.priceLab.text = [NSString stringWithFormat:@"¥ %0.2f", [costObj.discount floatValue] * self.buyNum];
                                self.payPrice = [NSNumber numberWithFloat:[costObj.discount floatValue] * self.buyNum];
                                self.isNotHidden = YES;
                            }
                        }
                    }
                    break;
            ...
            if ([costObj.payMode intValue] == 1) {
                ...
                self.priceLab.text = [NSString stringWithFormat:@"¥ %0.2f",[costObj.subscription floatValue]*self.buyNum];
                
                self.payPrice = [NSNumber numberWithFloat:[costObj.subscription floatValue]*self.buyNum];
            }else{
...
- (void) loadViews{
            ...
            for (int i = 0; i < self.obj.retData.packageInfo.dataList.count; i++) {
                ...
                if ([costObj.currentID intValue] == [self.packageID intValue]) {
                    ...
                    if ([self.obj.retData.isEnjoyDiscount intValue] == 1) {
                        if ([self.obj.retData.is_discount intValue] == 1) {/*存在折扣*/
                            if ([costObj.discount intValue] != 0) {
                                servePrice.text = [NSString stringWithFormat:@"¥ %0.2f", [costObj.discount floatValue] * self.buyNum];
                                self.isNotHidden = YES;
                            }
                        }
                    }
    ...
    self.pointsNum =[[UILabel alloc] init];
    self.pointsNum.textColor = [UIColor CMLUserBlackColor];
    self.pointsNum.font = KSystemFontSize12;
    ...
    if (self.packageID) {
        for (int i = 0; i < self.obj.retData.packageInfo.dataList.count; i++) {
            ...
            if ([costObj.currentID intValue] == [self.packageID intValue]) {
                ...
                self.pointsNum.text = [NSString stringWithFormat:@"%@",costObj.point];
    ...
    self.pointsNum.frame = CGRectMake(WIDTH - 30*Proportion - self.pointsNum.frame.size.width,
                                 CGRectGetMaxY(finalPay.frame) + 20*Proportion,
                                 self.pointsNum.frame.size.width,
                                 self.pointsNum.frame.size.height);
    [self.mainScrollView addSubview:self.pointsNum];
    ...
    if ([self.obj.retData.is_deposit intValue] == 1) {
        ...
        payTotalAmountNumLab.text = [NSString stringWithFormat:@"¥%0.2f", [self.obj.retData.deposit_money floatValue]*self.buyNum];
        ...
        if (self.packageID) {
            for (int i = 0; i < self.obj.retData.packageInfo.dataList.count; i++) {
                ...
                if ([costObj.currentID intValue] == [self.packageID intValue]) {
                    ...
                    payTotalAmountNumLab.text = [NSString stringWithFormat:@"¥%0.2f", [costObj.totalAmount floatValue]*self.buyNum];
                    
                    /*可享折扣*/
                    if ([self.obj.retData.isEnjoyDiscount intValue] == 1) {
                        if ([self.obj.retData.is_discount intValue] == 1) {/*存在折扣*/
                            if ([costObj.discount intValue] != 0) {
                                payTotalAmountNumLab.text = [NSString stringWithFormat:@"¥ %0.2f", [costObj.discount floatValue] * self.buyNum];
                                self.isNotHidden = YES;
                            }
                        }
                    }
            ...
            if ([costObj.payMode intValue] == 1) {
                ...
                payTotalAmountNumLab.text = [NSString stringWithFormat:@"¥%0.2f", [costObj.subscription floatValue]*self.buyNum];
            }else{
    ...
    self.couponContentLabel.text = @"有可用优惠券";//self.obj.retData.couponsName;
    ...
    /*可用积分*/
    UIView *pointBgView = [[UIView alloc] init];
    pointBgView.backgroundColor = [UIColor CMLWhiteColor];
    pointBgView.frame = CGRectMake(0,
                                   CGRectGetMaxY(newAddView.frame) + CGRectGetHeight(self.couponView.frame),
                                   WIDTH,
                                   74*Proportion);
    [self.mainScrollView addSubview:pointBgView];
    
    UILabel *pointPromLab = [[UILabel alloc] init];
    pointPromLab.text = self.pointDeductionStr;/*可用6000积分可抵扣300元*/
    ...
    [pointBgView addSubview:pointPromLab];
    ...
    [pointBgView addSubview:self.pointSwitch];
    ...
    /*是否显示积分*/
    if (self.obj.retData.couponsInfo || self.isNotHidden) {
        pointBgView.hidden = YES;
        pointBgView.frame = CGRectZero;
    }else {
        pointBgView.hidden = NO;
        pointBgView.frame = CGRectMake(0,
                                       CGRectGetMaxY(newAddView.frame) + CGRectGetHeight(self.couponView.frame),
                                       WIDTH,
                                       74*Proportion);
    ...
    if (self.invoiceSwitch.on) {
        ...
        self.invoiceBgView.frame = CGRectMake(0,
                                              CGRectGetHeight(pointBgView.frame) + CGRectGetMaxY(newAddView.frame) + CGRectGetHeight(self.couponView.frame),
                                              WIDTH,
...
-  (void) setRequestOfCreateOrder{
            ...
            if ([costObj.currentID intValue] == [self.packageID intValue]) {
                ...
                if ([self.obj.retData.isEnjoyDiscount intValue] == 1) {
                    ...
                    if ([self.obj.retData.is_discount intValue] == 1) {
                        if ([costObj.discount intValue] != 0) {
                            [paraDic setObject:[NSNumber numberWithInt:([costObj.discount intValue]*self.buyNum)*100] forKey:@"payAmtE2"];
                        }
                    }
                }
    ...
    if (self.chooseCouponsModel) {
        ...
        NSLog(@"couponsId%@", self.chooseCouponsModel.currentID);
    }
...
- (void) requestFailBack:(id)errorResult
             withApiName:(NSString *)apiName{
    NSLog(@"OrderCreate-error%@", errorResult);
    [self stopIndicatorLoading];
...
- (void)enterCouponBtnClicked {
    ...
    chooseCouponsVC.detailObj = self.obj;
    NSLog(@"chooseCouponsVC.row %ld", (long)self.row);
    NSLog(@"chooseCouponsVC.price %@", chooseCouponsVC.price);
    NSLog(@"chooseCouponsVC.objId %@", chooseCouponsVC.objId);
