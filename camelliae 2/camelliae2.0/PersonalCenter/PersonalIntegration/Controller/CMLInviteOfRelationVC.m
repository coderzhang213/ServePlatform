//
//  CMLInviteOfRelationVC.m
//  camelliae2.0
//
//  Created by 卡枚连 on 2019/8/22.
...
@interface CMLInviteOfRelationVC ()<NavigationBarProtocol,NetWorkProtocol>
...
@implementation CMLInviteOfRelationVC
...
- (void)shareMessage:(UIButton *)btn {
    ...
    if ([[[DataManager lightData] readUserLevel] intValue] != 1 || [[[DataManager lightData] readRoleId] intValue] > 4) {
        ...
        self.baseShareLink = [NSString stringWithFormat:@"http://m.camelliae.com/integral/invite-friends?inviteCode=k%@&level=2", [[DataManager lightData] readUserID]];
        self.baseShareTitle = self.obj.retData.title;
        self.baseShareContent = self.obj.retData.desc;
        [self changeShareStyle:btn];
        
    }else{
        [self showFailTemporaryMes:@"分享失败！"];
    }
}
