...
- (void)setTimeLabelValues:(double)currentTime totalTime:(double)totalTime {
    ...
    if ([self.videoShowTime floatValue] != 0) {
        
        if (currentTime >= [self.videoShowTime floatValue]) {
            
            self.dimissCompleteBlock();
            self.dimissCompleteAndBuyBlock();
            [self dismiss];
        }
    ...
    if ([[timeElapsedString stringByReplacingOccurrencesOfString:@":" withString:@""] intValue] == ([[timeRmainingString stringByReplacingOccurrencesOfString:@":" withString:@""] intValue] - 1) && ![timeRmainingString isEqualToString:@"00:00"]) {
