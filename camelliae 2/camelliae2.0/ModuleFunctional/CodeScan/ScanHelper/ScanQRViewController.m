...
@interface ScanQRViewController ()<CMLWalletCenterNavViewDelegate>
...
- (void)drawTitle {
    if (!_topTitle)
    {
        ...
        _topTitle.frame = CGRectMake(CGRectGetWidth(self.view.frame)/2 - 145/2, NavigationBarHeight + StatusBarHeight, 145, 60);
...
- (void)openOrCloseFlash {
    ...
    if (self.isOpenFlash)
    {
        [_btnFlash setImage:[UIImage imageNamed:@"CodeScan.bundle/qrcode_scan_btnk_flash_down"] forState:UIControlStateNormal];
    }
    else
        [_btnFlash setImage:[UIImage imageNamed:@"CodeScan.bundle/qrcode_scan_btn_flash_nor"] forState:UIControlStateNormal];
}
