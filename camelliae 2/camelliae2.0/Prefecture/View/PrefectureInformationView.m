...
- (void) loadViews{
    ...
    NSLog(@"%d", [self.obj.retData.newsInfoModule.dataCount intValue]);
    NSLog(@"%ld", self.dataArray.count);
    if ([self.obj.retData.newsInfoModule.dataCount intValue] <= self.dataArray.count) {
        moreBtn.hidden = YES;
    }
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        if (obj.hitNum) {
            ...
            [hitNum setTitle:@"0" forState:UIControlStateNormal];
        }
